...
from app.sim.orders import Order, faces_own_base, objective_position
...
class Formation:
    ...
    offsets: tuple[Vec2, ...]
    #: The deepest rank, measured back from the front rank.
    depth: float
...
def derive_formation(
    ...
    return Formation(
        offsets=tuple(Vec2(across, _lane(facing, depth)) for across, depth in [*mages, *summons]),
        depth=max((depth for _, depth in [*mages, *summons]), default=0.0),
        columns=summon_columns,
    )


def deployment_band(config: MapConfig, side: Side, troop_index: int, troop_count: int) -> Span:
    """The slice of the deployment strip one troop starts in.

    Troops share the strip side by side rather than stacking, so that three
    troops given three different orders start as three readable blocks.
    """
    strip = config.deployment[side]
    width = (strip.extent.end - strip.extent.start) / max(1, troop_count)
    start = strip.extent.start + width * troop_index
...
def deployment_anchor(
    ...
    """Where the troop's front rank stands at the opening whistle.

    One rule covers all five orders: the troop starts on the side of the
    deployment strip nearest its objective. Hold and Push have their objective
    out in front, so they are pressed against the far edge; Defend base's
    objective is behind it, so it gives up all the strip's free depth and sits
    back against its own base. Either way the summons are the rank facing the
    enemy — a defending troop faces outward, it does not turn around.
    ...
    return Vec2((band.start + band.end) / 2, far_edge - (inset + given_up) * forward(side))
